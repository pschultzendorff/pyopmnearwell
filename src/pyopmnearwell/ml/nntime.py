...
FEATURE_TO_INDEX: dict[str, int] = {
    ...
    "time_days": 13,
    "PI_analytical": 14,
    # Backwards-compatible aliases used in older experiments.
...
def scale_and_prepare_dataset(
    ...
    **kwargs,
    ...
    logger.info("Splitting data into train/val/test by member_id")

    map_file = pathlib.Path(dsfile) / "row_to_run_map.csv"
    if not map_file.exists():
        raise FileNotFoundError(f"Missing {map_file}; cannot split by member_id.")

    with map_file.open("r", newline="", encoding="utf-8") as f:
        row_map = list(csv.DictReader(f))

    row_members = np.array([int(r["member_id"]) for r in row_map])

    if len(row_members) != len(features):
        raise ValueError(
            f"row_to_run_map.csv has {len(row_members)} rows, "
            f"but dataset has {len(features)} rows."
        )

    all_members = np.array(sorted(set(row_members)))

    rng = np.random.default_rng(kwargs.get("seed", 19123))
    rng.shuffle(all_members)

    n_train = round(train_split * len(all_members))
    n_val = round(val_split * len(all_members))  # type: ignore

    train_members = set(all_members[:n_train])
    val_members = set(all_members[n_train : n_train + n_val])
    test_members = set(all_members[n_train + n_val :])

    train_mask = np.isin(row_members, list(train_members))
    val_mask = np.isin(row_members, list(val_members))
    test_mask = np.isin(row_members, list(test_members))
    ...
    train_rows = [r for r, keep in zip(row_map, train_mask) if keep]
    val_rows = [r for r, keep in zip(row_map, val_mask) if keep]
    test_rows = [r for r, keep in zip(row_map, test_mask) if keep]
    ...
    print("train runs:", sorted(train_members))
    print("val runs:  ", sorted(val_members))
    print("test runs: ", sorted(test_members))
...
def build_model(
    ...
    # Get hyperparameters.
    depth: int = hp.Int("depth", min_value=3, max_value=8, step=1)
...
def restructure_data(
    ...
    """Flatten the raw upscaled dataset into the training dataset used by the NN.

    Raw feature order from ``upscale.py`` is expected to be:
    pressure, saturation, radius, total_injected_volume, injection_rate,
    current_injection_time, previous_shutin_time, previous_injection_time,
    older_history_time, time_days, PI_analytical.
    """
    ...
    # Global / engineered features from raw upscaled dataset.
    RADIUS_IDX = 2
    FGIT_IDX = 3
    WGIR_IDX = 4
    CURR_INJ_IDX = 5
    ...
    new_features_lst.append(features[..., RADIUS_IDX])
    new_features_lst.append(features[..., FGIT_IDX])
    new_features_lst.append(features[..., WGIR_IDX])
    new_features_lst.append(features[..., CURR_INJ_IDX])
    new_features_lst.append(features[..., PREV_SHUT_IDX])
    new_features_lst.append(features[..., PREV_INJ_IDX])
    new_features_lst.append(features[..., OLDER_HIST_IDX])
    new_features_lst.append(features[..., TIME_IDX])
    ...
    X = new_features.reshape(-1, new_features.shape[-1])[valid]
    y_out = y_out[valid]
